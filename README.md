...
- `scripts/extract_excel_json.py`: 把 Excel 中的 sheet 转成 JSON
- `scripts/kconfig_to_json.py`: 把 `Kconfig/.config` 转成 JSON
- `scripts/build_render_context.py`: 合并 Excel JSON 和 Kconfig JSON，生成 `render_context.json`
- `scripts/render_app_config.py`: 读取 `render_context.json`，渲染 `app_config.c/.h`
- `scripts/run_pipeline.py`: 按顺序自动执行以上完整转换流程

当前目录结构按职责拆分为：

- `scripts/`: Python 脚本入口
- `resources/templates/`: `.c/.h` 模板和模板说明
- `resources/linker/`: `SCons` 使用的 linker script
- `docs/`: 独立文档
- `build/`: `SCons` 构建输出
- `output/`: 配置生成链输出
...
```powershell
uv run python scripts/extract_excel_json.py --config <config路径> --workbook <excel路径> --sheet <sheet名> --output <json路径>
```
...
```powershell
uv run python scripts/extract_excel_json.py --sheet CH_Cfg
```
...
```powershell
uv run python scripts/extract_excel_json.py --sheet "Unlock Mode1"
```
...
```powershell
uv run python scripts/extract_excel_json.py --sheet Motor_Cfg
```

转换 `TI_sequential`:

```powershell
uv run python scripts/extract_excel_json.py --sheet TI_sequential
```
...
当前 `scripts/build_render_context.py` 会优先填充模板中能直接推导出来的标量占位符。
对于还没有完成业务映射的大段 C 代码块，占位符会先落成 `TODO` 注释 stub，方便后续逐步替换成真实生成逻辑。

#### 第四步：JSON 渲染 app_config

`scripts/render_app_config.py` 不直接解析 Excel，也不直接理解 Kconfig。
它只负责读取“已经合并好的 render context JSON”，然后渲染 [app_config.h.tpl](resources/templates/app_config.h.tpl) 和 [app_config.c.tpl](resources/templates/app_config.c.tpl)。
...
- `--header-template`: `.h` 模板路径，默认 `resources/templates/app_config.h.tpl`
- `--source-template`: `.c` 模板路径，默认 `resources/templates/app_config.c.tpl`
- `--header-output`: 输出头文件路径，默认 `output/app_config.h`
- `--source-output`: 输出源文件路径，默认 `output/app_config.c`

`scripts/build_render_context.py` 生成的 `render_context.json` 会同时保留原始 `excel` / `kconfig` 数据，以及模板渲染用的 `placeholders` / `sections`。
render context JSON 推荐结构可参考 [templates README](resources/templates/README.md)。
...
- 如果启用这个方案，需要同时调整 `scripts/build_render_context.py`、`resources/templates/app_config.h.tpl` 和 `resources/templates/app_config.c.tpl`，不能只修改 `CHCM_CFG_IDX_MAX` 一个值。
